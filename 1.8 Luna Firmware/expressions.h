...
#include "qr_card.h"   // needed so LunaQR type is known + extern qrCard
#include "wallpaper_image.h" // Custom wallpaper PROGMEM array (generated by app)

// External references injected from main .ino (defined after audio.h is included there)
extern bool gamesActive;
extern bool gamePlaying;
extern int gameSelected;
extern LunaQR qrCard;  // Digital Business Card QR manager

// Forward-declare LunaAudio so we can hold an extern reference without re-including audio.h
class LunaAudio;
...
public:
  ...
  void drawStatusBar(int hour, int minute) {
    ...
    // Left-ish: BLE connection status dot (next to time to avoid battery overlap)
    uint16_t bleC = bleConnectedStatus ? (uint16_t)0x001F : (uint16_t)0xB5B6;
    display.fillCircle(38, 10, 2, bleC);

    // Right: battery percentage & battery icon
    int pct = 0;
    if      (batteryVolts >= 4.15f) pct = 100;
    else if (batteryVolts >= 4.05f) pct = 90 + (batteryVolts - 4.05f) * 100;
    else if (batteryVolts >= 3.95f) pct = 80 + (batteryVolts - 3.95f) * 100;
    else if (batteryVolts >= 3.87f) pct = 70 + (batteryVolts - 3.87f) * 125;
    else if (batteryVolts >= 3.82f) pct = 60 + (batteryVolts - 3.82f) * 200;
    else if (batteryVolts >= 3.79f) pct = 50 + (batteryVolts - 3.79f) * 333;
    else if (batteryVolts >= 3.75f) pct = 40 + (batteryVolts - 3.75f) * 250;
    else if (batteryVolts >= 3.72f) pct = 30 + (batteryVolts - 3.72f) * 333;
    else if (batteryVolts >= 3.68f) pct = 20 + (batteryVolts - 3.68f) * 250;
    else if (batteryVolts >= 3.60f) pct = 10 + (batteryVolts - 3.60f) * 125;
    else if (batteryVolts >= 3.30f) pct = (batteryVolts - 3.30f) * 33;
    else                             pct = 0;
    if (pct > 100) pct = 100;
    if (pct < 0)   pct = 0;

    // Draw battery percentage text (charcoal text, white bg)
    char batStr[8];
    snprintf(batStr, sizeof(batStr), "%d%%", pct);
    int batW = strlen(batStr) * 6;
    display.setTextColor(barText, barBg);
    display.setCursor(105 - batW, 6);
  ...
  void drawSettingsMenuLandscape(int option, bool selected, bool bleOn, int speed, int clockStyle, bool invertOn, int brightness) {
    ...
    // Header label
    display.setTextSize(1);
    display.setTextColor(0xFDA0, themeBg);
    display.setCursor(12, 34);
    display.print("--- SETTINGS ---");
    display.drawFastHLine(12, 44, SCREEN_WIDTH - 24, themeBorder);

    int itemsPerPage = isSmall ? 3 : 5;
    ...
    for (int pageIdx = 0; pageIdx < itemsPerPage; pageIdx++) {
      ...
      switch (optIdx) {
        ...
        case 5:
          ...
          display.print(silentMode ? "MUTED" : "ON");
          break;
        case 6:
    ...
    if (settingsActive) {
      ...
      display.setCursor((SCREEN_WIDTH - hint.length() * 6) / 2, SCREEN_HEIGHT - (isSmall ? 18 : 22));
      display.print(hint);
    }
  }
  ...
  void drawMapScreenLandscape(int hour, int minute, bool is12Hour) {
    ...
    // Clear whole screen
    display.fillRect(0, 0, 128, 160, themeBg);

    // 1. Top Card for instruction
    display.fillRoundRect(6, 6, 116, 42, 6, (themeAccent == 0x001F) ? 0xE7FC : 0xFDF2);
    display.drawRoundRect(6, 6, 116, 42, 6, themeAccent);

    // Direction Label inside card
    ...
    display.setTextSize(1);
    display.setTextColor(themeAccent);
    display.setCursor((128 - lblW) / 2, 12);
    display.print(dirLabel);

    // Street description inside card
    String desc = mapDescription;
    if (desc.length() > 16) desc = desc.substring(0, 14) + "..";
    int descW = desc.length() * 6;
    display.setTextColor(themeText);
    display.setCursor((128 - descW) / 2, 28);
    display.print(desc);

    // 2. Arrow Center (y centered around 92)
    int cx = 64;
    int cy = 92;

    // Draw clean vector arrow
    if (mapDirection.indexOf("LEFT") >= 0) {
      display.fillRect(cx + 8, cy - 2, 8, 22, themeAccent);
      display.fillRect(cx - 12, cy - 10, 28, 8, themeAccent);
      display.fillTriangle(cx - 20, cy - 6, cx - 10, cy - 14, cx - 10, cy + 2, themeAccent);
    } else if (mapDirection.indexOf("RIGHT") >= 0) {
      display.fillRect(cx - 16, cy - 2, 8, 22, themeAccent);
      display.fillRect(cx - 16, cy - 10, 28, 8, themeAccent);
      display.fillTriangle(cx + 20, cy - 6, cx + 10, cy - 14, cx + 10, cy + 2, themeAccent);
    } else if (mapDirection.indexOf("UTURN") >= 0 || mapDirection.indexOf("U-TURN") >= 0) {
      display.fillRect(cx - 14, cy - 6, 8, 26, themeAccent);
      display.fillRect(cx + 6, cy - 6, 8, 16, themeAccent);
      display.fillRect(cx - 14, cy - 14, 28, 8, themeAccent);
      display.fillTriangle(cx + 10, cy + 18, cx + 2, cy + 10, cx + 18, cy + 10, themeAccent);
    } else if (mapDirection.indexOf("ROUNDABOUT") >= 0 || mapDirection.indexOf("ROUND") >= 0) {
      display.drawCircle(cx, cy, 12, themeAccent);
      display.drawCircle(cx, cy, 11, themeAccent);
      display.fillCircle(cx, cy, 8, themeBg);
      display.fillRect(cx + 8, cy - 4, 12, 8, themeAccent);
      display.fillTriangle(cx + 26, cy, cx + 20, cy - 7, cx + 20, cy + 7, themeAccent);
      display.fillCircle(cx, cy, 3, themeAccent);
    } else {
      // STRAIGHT
      display.fillRect(cx - 4, cy - 10, 8, 30, themeAccent);
      display.fillTriangle(cx, cy - 22, cx - 12, cy - 8, cx + 12, cy - 8, themeAccent);
    }

    // 3. Bottom Section for distance
    String distStr = (mapDistance == "" || mapDistance == "--") ? "---" : mapDistance;
    display.setTextSize(3);
    display.setTextColor(themeAccent);
    int distW = distStr.length() * 18;
    display.setCursor((128 - distW) / 2, 132);
  ...
  void drawClockScreen(int hour, int minute, int second, String day, String date, int style, bool is12Hour, int steps) {
    ...
    // Clear content area ONLY if not using custom wallpaper
    if (style != 3) {
    ...
    if (style == 0) {
      ...
      if (WALLPAPER_IS_DEFAULT == 0) {
        display.drawRGBBitmap(0, 0, (const uint16_t*)wallpaper_data, WALLPAPER_WIDTH, WALLPAPER_HEIGHT);
      } else {
        display.fillRect(0, 22, 128, 138, bg);
      }

      // Large Digital Time in Center with Drop Shadow
      char tBuf[6]; snprintf(tBuf, sizeof(tBuf), "%02d:%02d", dispH, minute);
      display.setTextSize(3);
      
      int timeW = 5 * 18; // 5 characters * 18 pixels per character at size 3
      int timeX = (128 - timeW) / 2;
      int timeY = 56;
      
      // Shadow
      display.setTextColor(TFT_BLACK);
      display.setCursor(timeX + 2, timeY + 2);
      display.print(tBuf);
      // Main text (White)
      display.setTextColor(TFT_WHITE);
      ...
      char secBuf[4]; snprintf(secBuf, sizeof(secBuf), "%02d", second);
      display.setTextSize(1);
      display.setCursor(timeX + timeW + 4, timeY + 14);
      display.setTextColor(TFT_BLACK);
      display.print(secBuf);
      display.setCursor(timeX + timeW + 3, timeY + 13);
      display.setTextColor(accent);
      display.print(secBuf);
      
      // AM/PM if 12 hour
      if (is12Hour) {
        display.setCursor(timeX + timeW + 4, timeY);
        display.setTextColor(TFT_BLACK);
        display.print(ampm);
        display.setCursor(timeX + timeW + 3, timeY - 1);
        display.setTextColor(TFT_WHITE);
        display.print(ampm);
      }

      // Separator / Info card at bottom
      display.fillRoundRect(8, 102, 112, 44, 6, (robotVariant == "mr_luna") ? 0x000F : 0x7800); // dark semi-trans visual block
      ...
      // Steps
      display.setCursor(16, 126);
      display.print("Steps: " + String(steps));

      // Draw status bar over full screen wallpaper
      if (WALLPAPER_IS_DEFAULT == 0) {
  ...
  void draw(int hour, int minute, int second, String day, String date, int style = 0, bool is12Hour = false) {
    // Removed dual-button exit check to prevent accidental exit during game combo actions

    bool hasWallpaper = false;
    if (currentScreen == SCREEN_CLOCK && style == 3) {
      File f = SPIFFS.open("/wallpaper.bin", "r");
      if (f) {
        f.read((uint8_t*)display.getBuffer(), SCREEN_WIDTH * SCREEN_HEIGHT * 2);
        f.close();
        hasWallpaper = true;
      }
    }
      ...
      switch (currentScreen) {
        ...
        case SCREEN_GAMES:
          if (!gamesActive) {
            ...
            display.drawRoundRect(44, 54, 40, 24, 6, themeAccent);
            display.fillCircle(54, 66, 2, themeAccent);
            display.fillCircle(74, 66, 2, themeText);

            // Title
            display.setTextSize(1);
            display.setTextColor(themeText);
            String titleStr = "LUNA ARCADE";
            display.setCursor((128 - titleStr.length() * 6) / 2, 96);
            display.print(titleStr);

            // Divider
            display.drawFastHLine(6, 126, 116, themeBorder);
