...
// Compatible with 1.8" ST7735 (128×160)
// =============================================================================
#ifndef QR_CARD_H
#define QR_CARD_H
...
public:
  ...
  void drawQRScreen(GFXcanvas16& display) {
    ...
    // Available vertical space: SCREEN_HEIGHT minus bottom label (14px) minus top margin (4px)
    int availH = SCREEN_HEIGHT - 18;
    int availW = SCREEN_WIDTH;

    // Use 2-module quiet zone to maximise pixel size on the small screen
    int quietModules = 2;
    int totalModules = qrcode.size + quietModules * 2;

    // Choose module pixel size (1–4 px) — biggest that fits
    ...
    display.setCursor((SCREEN_WIDTH - labelW) / 2, SCREEN_HEIGHT - 12);
    display.print(label);
  }
...
private:
  void _drawNoCardScreen(GFXcanvas16& display) {
    ...
    display.println("  No Business Card");
    display.setCursor(8, 68);
    display.println("  Open Luna app and");
    display.setCursor(8, 82);
    display.println("  sync your QR card.");
  }
